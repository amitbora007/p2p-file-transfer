...
const alertVariants = cva(
  "relative w-full rounded-2xl border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        default: "bg-slate-900/90 border-slate-800/80 text-slate-200 backdrop-blur-xl shadow-xl",
        destructive:
          "text-rose-200 bg-rose-950/40 border-rose-900/50 [&>svg]:text-rose-400 *:data-[slot=alert-description]:text-rose-200",
      },
    },
    defaultVariants: {
      variant: "default",
